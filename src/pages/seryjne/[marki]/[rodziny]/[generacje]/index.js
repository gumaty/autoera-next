...
export async function getServerSideProps(context) {
	...
	const uniqueCarBodies = [
		...new Set(nadwozia.map((object) => JSON.stringify(object))),
	...
	const models = uniqueCarBodies.map((carBody, index) => {
		...
		for (let i = 0; i < modele.length; i++) {
			if (
				modele[i].typ_nadwozia === carBody.typ_nadwozia &&
				modele[i].liczba_drzwi === carBody.liczba_drzwi
			) {
